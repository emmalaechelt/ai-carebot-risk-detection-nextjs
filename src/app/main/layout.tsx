...
interface MenuItem { label: string; icon?: string; href?: string; children?: MenuItem[]; }
        ...
        return (
          <li key={item.label}>
            {item.children ? (
              ...
              <Link href={item.href || "#"} className={`flex items-center space-x-2 px-1 py-1 rounded-lg transition
      ...
      <main className="flex-1 flex flex-col">
        <header className="flex items-center justify-end h-16 px-6 bg-white border-b border-gray-300">
          <div ref={dropdownRef} className="relative">
            ...
            <div className={`absolute top-full right-0 mt-2 w-36 bg-white border border-gray-300 rounded-lg shadow-md
              transform transition-all duration-200 origin-top-right ${isDropdownOpen ? "opacity-100 scale-100 translate-y-0" : "opacity-0 scale-95 -translate-y-2 pointer-events-none"}`}>
              <button onClick={handleLogout} className="block w-full px-4 py-2 text-sm text-red-600 hover:bg-gray-100 rounded-lg">
                로그아웃
              </button>
            </div>
          </div>
